...
module.exports = {
    ...
    userhelpers.getAllProducts().then((products) => {
      ...
      for (let i in products) {
        if (products[i].products.discount) {
          products[i].products.discountPrice =
            products[i].products.price -
            (products[i].products.discount * products[i].products.price) / 100;
        } else {
  ...
  addToCart: (req, res) => {
    ...
    req.body.count = 1;
    userhelpers
      .addToCart(req.body, req.session.user._id, req.params.id)
      .then((response) => {
  ...
  productDetails: (req, res) => {
    ...
    sellerHelpers.viewProduct(req.params.id).then((product) => {
      if (req.session.useer) {
        userhelpers
          .isAcustomer(req.params.id, req.session.user._id)
          .then((isAcustomer) => {
  ...
  changeQuantity: (req, res) => {
    // console.log(req.body);
    userhelpers
      .changeQuantity(req.body, req.session.user._id)
      .then((response) => {
        // console.log(response);
        res.json({ status: true });
      });
    ...
    userhelpers.getAllBrands(gender).then((response) => {
      ...
      for (let i in filterResult) {
        if (filterResult[i].products.discount) {
          filterResult[i].products.discountPrice =
            filterResult[i].products.price -
            (filterResult[i].products.discount *
              filterResult[i].products.price) /
              100;
        } else {
          filterResult[i].products.discountPrice =
  ...
  placeOrder: (req, res) => {
    ...
    userhelpers
      .placeOrder(req.body, req.session.user, cartDetails)
      .then((orderId) => {
        orderid = orderId;
        // console.log(orderId);
  ...
  orderSuccess: (req, res) => {
    userhelpers.getOrderDetails(orderid).then((orderDetails) => {
      ...
      res.render('users/messages/order-success', {
        user: req.session.user,
    ...
    userhelpers.getAllBrands(gender).then((response) => {
      for (let i in SearchResult) {
        if (SearchResult[i].products.discount) {
          SearchResult[i].products.discountPrice =
            SearchResult[i].products.price -
            (SearchResult[i].products.discount *
              SearchResult[i].products.price) /
              100;
        } else {
          SearchResult[i].products.discountPrice =
  ...
  searchResult: (req, res) => {
    ...
    if (req.body.search) {
      ...
      ////////////////
      if (a.Sprice) {
        price = parseInt(a.Sprice);
      } else {
        price = parseInt(a.Sprice1);
      }
  ...
  filterProducts: (req, res) => {
    ...
    userhelpers.filterProducts(filter, gender, price).then((response) => {
      ...
      // console.log(filterResult);
      if (req.body.sort == 'Sort' || req.body.sort1 == 'Sort') {
        res.json({ status: true });
      }
      ...
      if (req.body.sort == 'hl' || req.body.sort == 'hl') {
        filterResult.sort((a, b) => {
          return b.products.price - a.products.price;
        });
        res.json({ status: true });
